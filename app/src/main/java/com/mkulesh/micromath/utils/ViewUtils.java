...
public final class ViewUtils {
    ...
    public static void debug(Object o, String text) {
        if (DLog.DEBUG) DLog.d(TAG, o.getClass().getSimpleName() + ": " + text);
    }
    ...
    public static int getLayoutDepth(LinearLayout l) {
        ...
        while (p != null) {
            ...
            if (p instanceof FormulaView && p.getParent() == null) {
                retValue += 2;
                break;
            }
            retValue++;
    ...
    public static String[] catValues(double[] values, int significantDigits) {
        ...
        for (int run = 0; run < 2; run++) {
            ...
            for (int pos = 0; pos <= maxLength; pos++) {
                StringBuilder format = new StringBuilder((pos < 1) ? "0" : "0.");
                for (int k = 0; k < pos; k++) {
                    format.append("0");
                }
                if (run == 1) {
                ...
                final DecimalFormat df = CompatUtils.getDecimalFormat(format.toString());
                trial.clear();
                boolean hasDuplicate = false;
                int trialLength = 0;
                for (int i = 0; i < values.length; i++) {
                    String fValue = (values[i] != 0.0) ? df.format(values[i]) : "0";
                    if (fValue != null && (format.toString().equals(fValue) || ("-" + format).equals(fValue))) {
                        fValue = "0";
                    }
                    strValues[i] = fValue;
                    if (!trial.add(fValue)) {
     ...
     */
    public static void setImageButtonColorAttr(Context context, ImageView b, @AttrRes int resId) {
